...
public class PlayerSuggestionField extends JTextField implements DocumentListener {
	...
	public PlayerSuggestionField(String s) {
		
		setText(s);
		
		players = FileManager.getPlayers();
		
		Collections.sort(players);
		
	}
	
	public static void setPlayers(ArrayList<String> p) {
		players.clear();
		players.addAll(p);
	}
	...
	public void insertUpdate(DocumentEvent arg0) {
		...
		int n = Collections.binarySearch(players, prefix);
		if(n < 0 && -n <= players.size()) {
			String match = players.get(-n - 1);
